...
function woodTexture() {
  ...
  grd.addColorStop(0, '#8a5a2b'); grd.addColorStop(0.5, '#9c6a34'); grd.addColorStop(1, '#7d4f26');
  g.fillStyle = grd; g.fillRect(0, 0, 128, 512);
  // vertical grain streaks
  g.globalAlpha = 0.35;
  const shades = ['#6b4420', '#5a3818', '#a3743f', '#734a24'];
  for (let i = 0; i < 60; i++) {
    const x = (i * 37 % 128);
    g.strokeStyle = shades[i % shades.length];
    g.lineWidth = 0.6 + (i % 3) * 0.7;
    g.beginPath();
    g.moveTo(x, 0);
    g.bezierCurveTo(x + 6, 170, x - 6, 340, x + 3, 512);
    g.stroke();
  }
  g.globalAlpha = 1;
...
function buildFrame(M) {
  ...
  const t = 15 * mPerMM;                                 // frame thickness (15 mm)
  const h = 15 * mPerMM;                                 // frame height (15 mm)
