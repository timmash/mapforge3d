...
// crust thickness of the terrain-colour cap over the base block, in metres,
// scaled so it stays a printable layer once the model is shrunk to 200 mm.
function terrainCrust() {
  return Math.min(20, Math.max(1.5, Math.max(EXT.hx, EXT.hy) * 2 / 300));
}
...
function buildTerrainBlock(groundAt) {
  ...
  const eps = terrainCrust();

  // grid of 2D vertices + triangulation (shared by both stacked solids)
  const verts = [], tris = [];
  const V = (i, j) => j * (N + 1) + i;
  for (let j = 0; j <= N; j++) for (let i = 0; i <= N; i++) verts.push([-hx + i * stepX, -hy + j * stepY]);
  for (let j = 0; j < N; j++) for (let i = 0; i < N; i++) {
    const a = V(i, j), b = V(i + 1, j), c = V(i + 1, j + 1), d = V(i, j + 1);
    tris.push(a, b, c, a, c, d);
  }
  const surf = (x, y) => groundAt(x, y);

  // BASE BLOCK: closed solid, surface-shaped top → flat bottom (base colour)
  const base = new THREE.Mesh(closedDrapedSolid(verts, tris, surf, () => bot), MATS.base);
  base.name = 'base';
  // TERRAIN CAP: thin closed slab hugging the surface (terrain colour), sits on the base
  const terrain = new THREE.Mesh(closedDrapedSolid(verts, tris, (x, y) => surf(x, y) + eps, surf), MATS.terrain);
...
// Terrain shaped to the council boundary: a stacked pair of watertight draped
// slabs per mask ring — a base block plus a thin terrain-colour cap on top.
function buildCouncilTerrain(groundAt) {
  ...
  for (const ringXY of EXT.mask) {
    ...
    // base block (surface top → flat bottom) + terrain cap (thin slab on the surface)
    const base = new THREE.Mesh(closedDrapedSolid(verts, tris, surf, () => bot), MATS.base);
    base.name = 'base';
    const terrain = new THREE.Mesh(closedDrapedSolid(verts, tris, (x, y) => surf(x, y) + eps, surf), MATS.terrain);
    terrain.name = 'terrain';
    group.add(base, terrain);
  }
  return group;
}
...
function writeColour3MF(root, filename) {
  ...
  root.traverse(o => {
    ...
    // weld coincident vertices by POSITION only (strip other attributes first, so
    // differing normals don't block the merge) → shared edges, no open-edge seams
    let welded;
    try {
      const po = new THREE.BufferGeometry();
      po.setAttribute('position', o.geometry.getAttribute('position').clone());
      if (o.geometry.index) po.setIndex(o.geometry.index.clone());
      welded = mergeVertices(po, 1e-3);
    } catch (e) { welded = o.geometry; }
    const pos = welded.attributes.position, idx = welded.index;
