...
function woodTexture() {
  ...
  const tones = ['#8a5a30', '#7c4f2a', '#976a3c', '#835327'];
  for (let p = 0; p < planks; p++) {
    const x0 = p * pw;
    // base plank colour with a soft cross-plank gradient
    const grd = g.createLinearGradient(x0, 0, x0 + pw, 0);
    grd.addColorStop(0, tones[p % tones.length]);
    grd.addColorStop(0.5, tones[(p + 1) % tones.length]);
    grd.addColorStop(1, tones[p % tones.length]);
    g.fillStyle = grd; g.fillRect(x0, 0, pw, S);
    // long grain lines running along the plank
    for (let i = 0; i < 22; i++) {
      const gx = x0 + ((i + 0.5) * (pw / 22)) + ((i * 17) % 6) - 3;
      g.strokeStyle = `rgba(50,30,14,${0.12 + (i % 4) * 0.05})`;
      g.lineWidth = 0.6 + (i % 3) * 0.6;
      g.beginPath(); g.moveTo(gx, -10);
      g.bezierCurveTo(gx + 7, S * 0.33, gx - 7, S * 0.66, gx + 4, S + 10);
      g.stroke();
    }
    // occasional knot
    if ((p % 2) === 0) {
      const ky = 90 + ((p * 137) % (S - 180));
      const kx = x0 + pw * (0.4 + ((p * 0.13) % 0.3));
      for (let r = 16; r > 0; r -= 3) {
        g.strokeStyle = `rgba(45,26,10,${0.5 - r * 0.02})`; g.lineWidth = 1.5;
        g.beginPath(); g.ellipse(kx, ky, r * 0.7, r, 0, 0, Math.PI * 2); g.stroke();
      }
    }
    // groove shadow + highlight between planks
    g.fillStyle = 'rgba(0,0,0,0.30)'; g.fillRect(x0, 0, 2.5, S);
    g.fillStyle = 'rgba(255,255,255,0.06)'; g.fillRect(x0 + 2.5, 0, 1.5, S);
  }
...
function brickTexture() {
  ...
  const c = document.createElement('canvas'); c.width = S; c.height = S;
  const g = c.getContext('2d');
  // mortar base with a little grain
  g.fillStyle = '#c8c1b2'; g.fillRect(0, 0, S, S);
  for (let i = 0; i < 6000; i++) { const x = (i * 71) % S, y = (i * 131) % S; g.fillStyle = `rgba(0,0,0,${0.03 + ((x + y) % 5) * 0.01})`; g.fillRect(x, y, 1, 1); }
  const bw = 120, bh = 40, m = 9;    // brick + mortar gap
  let row = 0;
  for (let y = -bh; y < S + bh; y += bh + m) {
    const off = (row % 2) ? -(bw + m) / 2 : 0;
    for (let x = off - bw; x < S + bw; x += bw + m) {
      // per-brick colour variation (warm reds/browns)
      const r = 150 + (((x * 13 + y * 7) % 55));
      const gg = 60 + (((x * 5 + y * 11) % 28));
      const b = 48 + (((x * 3 + y * 17) % 22));
      g.fillStyle = `rgb(${Math.min(195, r)},${gg},${b})`;
      g.fillRect(x, y, bw, bh);
      // bevel: top/left highlight, bottom/right shadow
      g.fillStyle = 'rgba(255,255,255,0.08)'; g.fillRect(x, y, bw, 3); g.fillRect(x, y, 3, bh);
      g.fillStyle = 'rgba(0,0,0,0.16)'; g.fillRect(x, y + bh - 3, bw, 3); g.fillRect(x + bw - 3, y, 3, bh);
      // speckle within the brick
      for (let k = 0; k < 22; k++) { const sx = x + ((k * 29) % bw), sy = y + ((k * 19) % bh); g.fillStyle = `rgba(0,0,0,0.05)`; g.fillRect(sx, sy, 2, 2); }
    }
    row++;
  }
  const tex = new THREE.CanvasTexture(c);
